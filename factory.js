(function () {
  ...
  function storiesComposePlaceholder() {
    return (site && site.composePlaceholder) || "What's happening on BART?";
  }
        ...
        showAddTray: function () {
          ...
          el.innerHTML =
            '<button type="button" class="stories-item is-add" data-story-add="1" aria-label="Add story">' +
              '<span class="stories-ring"><span class="stories-avatar" style="background:' + colorFor(SITE_ID || 'bart') + '">' +
              escapeHtml(String(SITE_ID || 'bart').replace(/chat$/i, '').slice(0, 3).toUpperCase() || 'ME') + '</span>' +
        ...
        setDemo: function (list) {
          liveStories = (list || []).map(function (s, i) {
            return {
              ...
              name: s.name || (site && site.name) || 'bartchat',
              handle: s.handle || SITE_ID || 'bart',
              type: s.type || 'text',
              text: s.text || '',
              mediaUrl: s.mediaUrl || '',
